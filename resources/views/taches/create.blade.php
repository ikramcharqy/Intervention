<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Créer une Tâche</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                <form method="POST" action="{{ route('taches.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="intervention_id" id="intervention_id" required
                            ...
                            @foreach ($interventions as $intervention)
                                <option value="{{ $intervention->id }}" @selected(old('intervention_id') == $intervention->id)>{{ $intervention->reference }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="ordre" id="ordre" min="1" value="{{ old('ordre', 1) }}"
                            class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                        @error('ordre')<p class="text-red-500 text-sm">{{ $message }}</p>@enderror
                    </div>

                    <div class="flex gap-3">
                        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Enregistrer</button>
                        <a href="{{ route('taches.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
