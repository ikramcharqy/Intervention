<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if($demandeIntervention->statut === 'En attente')
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6 border-2 border-indigo-500/30">
                    <h3 class="text-base font-extrabold text-indigo-700 dark:text-indigo-400 border-b border-gray-100 dark:border-gray-700 pb-3 mb-4 flex items-center space-x-2">
                        ...
                        <span>Qualification Commerciale (Échange Client)</span>
                    </h3>

                    <p class="text-xs text-gray-600 dark:text-gray-300 mb-4">
                        Consultez la demande du client, puis contactez-le par téléphone (<strong>{{ $demandeIntervention->client->telephone ?? 'N/A' }}</strong>) ou email (<strong>{{ $demandeIntervention->client->email ?? 'N/A' }}</strong>). Renseignez l'échange ci-dessous pour valider ou refuser la demande.
                    </p>
                    ...
                    <form action="{{ route('demande-interventions.validerConvertir', $demandeIntervention) }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Compte-rendu d'échange (Téléphone / Email)</label>
                                <textarea name="compte_rendu_echange" rows="3" placeholder="Ex: Client joint par téléphone. Demande confirmée pour le site principal..." class="w-full text-xs rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 focus:ring-indigo-500 shadow-sm"></textarea>
                            </div>
                            <div>
