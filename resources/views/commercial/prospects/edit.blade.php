<x-commercial-layout>
    ...
    <div class="w-full space-y-5">
        ...
        <div class="flex items-center justify-between">
            <div>
                <a href="{{ route('prospects.show', $prospect) }}" class="text-xs font-semibold inline-flex items-center gap-1.5 text-[#5E6278] hover:text-[#181C32]">
                    <i class="fas fa-arrow-left text-[10px]"></i> {{ $prospect->nom_entreprise }}
                </a>
                <h1 class="text-lg font-extrabold text-[#181C32] font-heading mt-1">Modifier le Prospect</h1>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('prospects.show', $prospect) }}" class="px-3.5 py-2 text-[#5E6278] text-xs font-bold hover:text-[#181C32] transition">Annuler</a>
        ...
        @php
            $selectedTypes = old('type_intervention_ids', $prospect->typesIntervention->pluck('id')->all());
            $selectedTypeProspect = old('type_prospect', $prospect->type_prospect);
        @endphp

        <form id="prospect-form" action="{{ route('prospects.update', $prospect) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="metronic-card p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <select name="commercial_id" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                ...
                                @foreach($commerciaux as $c)
                                    <option value="{{ $c->id }}" {{ old('commercial_id', $prospect->commercial_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <input type="text" name="nom_entreprise" required value="{{ old('nom_entreprise', $prospect->nom_entreprise) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="nom_contact" value="{{ old('nom_contact', $prospect->nom_contact) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $prospect->email) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                        @error('email')<p class="text-rose-600 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-[#181C32] mb-1.5">Téléphone</label>
                        <input type="text" name="telephone" value="{{ old('telephone', $prospect->telephone) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500" placeholder="0612345678">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="adresse" value="{{ old('adresse', $prospect->adresse) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                    </div>
                </div>
            </div>
            ...
            <div class="metronic-card p-6">
                ...
                <div class="grid grid-cols-2 sm:w-1/2 gap-3" id="type-prospect-cards">
                    @php $typeProspectIcons = ['Entreprise' => 'fa-building', 'Particulier' => 'fa-user']; @endphp
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="statut" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                            @foreach(['Nouveau', 'Qualifié', 'Négociation', 'Converti', 'Perdu'] as $s)
                                <option value="{{ $s }}" {{ old('statut', $prospect->statut) == $s ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="prochaine_action_date" value="{{ old('prochaine_action_date', $prospect->prochaine_action_date?->format('Y-m-d')) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-[#181C32] mb-1.5">Prochaine action — description</label>
                        <input type="text" name="prochaine_action_description" value="{{ old('prochaine_action_description', $prospect->prochaine_action_description) }}" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500" placeholder="Ex: Rappeler suite envoi devis">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="observations" rows="4" class="w-full text-sm border border-[#EFF2F5] rounded-lg px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-emerald-500">{{ old('observations', $prospect->observations) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition">
                    <i class="fas fa-check mr-1.5"></i> Enregistrer les modifications
                </button>
            </div>
        </form>
    </div>
